class SkOauth
  ...
  def auth_dialog
    "#{sk_url}/oauth/authorize?client_id=#{@app_id}&redirect_uri=#{@app_redirect_url}&scope=#{@app_scope}"
  end
  ...
  # oauth/access token
  def get_token(code)
    url = "#{sk_url}/oauth/access_token?code=#{code}&client_id=#{@app_id}&client_secret=#{@app_secret}&redirect_uri=#{@app_redirect_url}"
